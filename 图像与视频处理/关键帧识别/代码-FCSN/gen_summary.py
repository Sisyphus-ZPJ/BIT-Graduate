...
def plot_bar():
    with open(anno_path) as f:
        ...
        for row in csv_reader:
            ...
            if str(idx//20+1) in ids:
                ...
                # if str(idx//20+1) == '36':
                #    print(csv_dict[str(idx//20+1)])
                #    print(np.min(csv_dict[str(idx//20+1)]))
            idx += 1
    
    # sns.set()
    fig, ax = plt.subplots(ncols=1, nrows=len(ids), figsize=(40, 30))
    fig.tight_layout()
    for id, axi in zip(ids, ax.flat):
        ...
        axi.bar(x=list(range(len(scores))), width=0.8, height=scores, color=['lightseagreen' if i == 0 else 'orange' for i in pred_summary], edgecolor=None)
        axi.set_title(id)
