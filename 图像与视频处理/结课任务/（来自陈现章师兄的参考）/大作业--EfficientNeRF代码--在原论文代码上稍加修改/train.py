...
class NerfTree_Pytorch(object):  # This is only based on Pytorch implementation
    ...
    def __init__(self, xyz_min, xyz_max, grid_coarse, grid_fine, deg, sigma_init, sigma_default, device):
        ...
        # 创建coarse阶段的缓冲
        self.sigma_voxels_coarse_temp = torch.full((grid_coarse, grid_coarse, grid_coarse),
                                                   self.sigma_init,
                                                   device=device)

        self.index_voxels_coarse = torch.full((grid_coarse, grid_coarse, grid_coarse),
                                              0,
                                              dtype=torch.long,
                                              device=device)
    ...
    def calc_index_coarse(self, xyz):
        ijk_coarse = ((xyz - self.xyz_min) / self.xyz_scope * self.grid_coarse).long().clamp(min=0,
                                                                                             max=self.grid_coarse - 1)
    ...
    def update_coarse(self, xyz, sigma, beta):
        ...
        # 更新前对要更新的数据查询temp
        temp_index1 = self.sigma_voxels_coarse_temp[ijk_coarse[:, 0], ijk_coarse[:, 1], ijk_coarse[:, 2]] <= 0
        # 查询在缓冲区内，但这次数据不为零的点
        temp_index2 = self.sigma_voxels_coarse_temp[ijk_coarse[:, 0], ijk_coarse[:, 1],
                                                    ijk_coarse[:, 2]] < self.T & (1 - temp_index1)
        self.sigma_voxels_coarse_temp[temp_index1] += 1
        # 获取大于阈值T的索引
        temp_index1 = self.sigma_voxels_coarse_temp >= self.T
        # 索引处数据归零
        self.sigma_voxels_coarse[temp_index1] = 0
        # 缓冲索引index2归零
        self.sigma_voxels_coarse_temp[temp_index2] = 0
    ...
    def create_voxels_fine(self):
        ijk_coarse = torch.logical_and(self.sigma_voxels_coarse > 0,
                                       self.sigma_voxels_coarse != self.sigma_init).nonzero().squeeze(1)  # (N, 3)
        ...
        self.voxels_fine = torch.zeros(num_valid,
                                       self.grid_fine,
                                       self.grid_fine,
                                       self.grid_fine,
                                       self.dim_sh + 1,
        ...
        # 创建fine阶段的temp
        self.voxels_fine_temp = torch.zeros(num_valid,
                                            self.grid_fine,
                                            self.grid_fine,
                                            self.grid_fine,
                                            self.dim_sh + 1,
    ...
    def update_fine(self, xyz, sigma, sh):
        ...
        # 进行缓存
        self.voxels_fine_temp[index_coarse, ijk_fine[:, 0], ijk_fine[:, 1], ijk_fine[:, 2]] = feat[nonzero_index_coarse]
        self.voxels_fine[index_coarse, ijk_fine[:, 0], ijk_fine[:, 1],
                         ijk_fine[:, 2]] = self.voxels_fine_temp[index_coarse, ijk_fine[:, 0], ijk_fine[:, 1],
                                                                 ijk_fine[:, 2]]
        self.voxels_fine_temp[nonzero_index_coarse <= 0] += 1
        temp_index = self.voxels_fine_temp >= self.T
        self.voxels_fine[temp_index] = 0
    ...
    def query_coarse(self, xyz, type='sigma'):
        ...
        # 查询大于给定阈值次数的索引，即无用点
        temp_index = self.sigma_voxels_coarse_temp[ijk_coarse[:, 0], ijk_coarse[:, 1], ijk_coarse[:, 2]] > self.T
        self.sigma_voxels_coarse[temp_index] = 0
    ...
    def query_fine(self, xyz):
        ...
        # fine阶段缓冲提取
        fine_data = self.voxels_fine[index_coarse, ijk_fine[:, 0], ijk_fine[:, 1], ijk_fine[:, 2]]
        fine_data[fine_data > 0] = self.voxels_fine_temp
        return fine_data
...
class EfficientNeRFSystem(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % 1000 == 0 and self.nerf_tree.voxels_fine == None:
            ...
            self.logger.experiment.add_figure('train/distribution', fig, self.global_step)
            plt.close()
    ...
    def validation_epoch_end(self, outputs):
        ...
        num_voxels_coarse = torch.logical_and(self.nerf_tree.sigma_voxels_coarse > 0,
                                              self.nerf_tree.sigma_voxels_coarse != self.sigma_init).nonzero().shape[0]
...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(f'logs/{hparams.exp_name}/ckpts', '{epoch:d}'),
        monitor='val/psnr',
        mode='max',
        save_top_k=5,
    )
